...
/*Apenas usada para debug*/
void printNull(int *sync){
	int i;
...
/*Apenas usada para debug*/
void printSync(int *sync){
	int i;
	for (i=0;i<INODE_TABLE_SIZE && sync[i]!=-1;i++)
		printf("%d ",sync[i]);
	printf("\n");
}
...
/*void organizeLocks(char* origin, char* destiny, char** first, char** last){

	int i;
	int slashesOrigin = 0, slashesDestiny = 0;
	int lenOrigin = strlen(origin), lenDestiny = strlen(destiny);

	if (origin[lenOrigin-1] == '/') {
		origin[lenOrigin-1] = '\0';
	}

	if (destiny[lenDestiny-1] == '/') {
		destiny[lenDestiny-1] = '\0';
	}
	...
	if (slashesOrigin<slashesDestiny){
		*first = origin;
		*last = destiny;
		return;
	}

	else if (slashesOrigin>slashesDestiny){
		*first = destiny;
		*last = origin;
		return;
	}
 ...
 */
void print_tecnicofs_tree(char *buffer){
